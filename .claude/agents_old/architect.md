...
- Tech stack: languages, frameworks, key libraries (with versions if discoverable), runtime requirements, external systems (databases, devices, cloud services)
...
FINAL STEPS:
1. If ARCHITECTURE.md already exists, Read it first. If the existing content is accurate, preserve and update. If outdated or wrong, overwrite completely with the corrected version.
2. Write the complete document in a SINGLE Write tool call.
3. After writing, briefly report: file location, approximate line count, and any uncertainties or gaps that warrant a follow-up.
