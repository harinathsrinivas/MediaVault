...
| `replace` | `replace [id]` | `cmd_replace` — swap original with dummy placeholder |
...
### 7.6 Placeholder / dummy-file system (`cmd_replace`, lines 762-813)

When the user is confident the upload is done, `cmd_replace` swaps the
large original for a tiny text dummy:

1. Refuse to act if `uploaded != True`.
2. Write a sibling file `<original>.temp_dummy` with content:
   ```
   Original Hash: <sha256>
   Status: SPLIT (Check filenames for .chunk.)     # only if split_info exists
   ```
   These dummies are < 1 KB.
3. Delete the original with **3 retries** and a `chmod S_IWRITE` to clear
   any read-only flag (`main.py:786-806`). Retries exist because Plex,
   Windows Search, or a video player may be holding the file open.
4. `os.rename(dummy, original)` — the dummy takes the exact name of the
   original. Plex/Kodi see a "file" with the same path and Windows still
   reports it in directory listings.
5. Set `status = "archived"`.

The 1024-byte threshold is reused by `cmd_prep` and `cmd_check` as a
sniff test: any file smaller than 1 KB is treated as a dummy and skipped.
