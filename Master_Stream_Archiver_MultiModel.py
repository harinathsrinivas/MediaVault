...
class MasterArchiverApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Golden Standard: Context-Aware Agentic Archiver v2")
    ...
    def build_ui(self):
        ...
        tk.Checkbutton(frame_ingest,
                       text="Override Duration Warnings (I verified match data is complete, missing time is just ads)",
    ...
    def orchestrate_analysis(self):
        ...
        # Fire warnings to client window immediately without crashing the AI generation thread
        if has_duration_warning and not self.var_ignore_dur.get():
            self.root.after(0, lambda: messagebox.showwarning("⚠️ DURATION DISCREPANCY DETECTED",
                                                              "One or more streams are significantly shorter than the primary record.\n\nThe algorithmic scorer penalized them automatically. The local AI team will now analyze the core streams to verify contextual completion."))

        # --- EXECUTE THE AGENT PIPELINE RUN ---
        if self.var_use_ollama.get() and not self.stop_requested:
            self.run_full_agent_pipeline()
        else:
    ...
    def run_full_agent_pipeline(self):
        ...
        for key, name in workers_list:
            ...
            if "Failed to connect" in response or "timed out" in response:
                ...
                worker_responses[
                    f"{name}_{model}"] = f"[ERROR: Model failed to respond due to endpoint connection timeout. Details: {response}]"
            else:
        ...
        # Execute Judge/Synthesizer Pass
        judge_key = "judge"
        ...
        algorithmic_report, _ = self.generate_verdict()
        self.root.after(0, self.cleanup_ui_post_run, algorithmic_report)

    def retry_single_agent(self, key):
        """Allows direct troubleshooting/re-execution of a specific broken local model container without wiping physical work."""
        url = self.ent_ollama_url.get().strip()
        model = self.model_entries[key].get().strip()
        ...
        def retry_thread():
            if key == "judge":
                # Build context out of current existing worker text tabs dynamically
                worker_responses = {}
    ...
    def parse_metrics(self, output):
        ...
        # REVERTED LOGIC: Trust the broadcast Duration header as the Ground Truth
        ...
        if dur_match:
            ...
            data["duration_str"] = f"{h:02d}h {m:02d}m {s:02d}s"
        else:
            data["duration_sec"] = 0
            data["duration_str"] = "Unknown"
    ...
    def generate_verdict(self):
        ...
        for name, data in self.results.items():
            ...
            report += f"[{name}]\n"
            report += f"  - Clean Calculated Duration: {data['duration_str']}\n"

            is_short = False
            if max_dur > 0 and data['duration_sec'] < (max_dur - 300):
                is_short = True
                if override_duration:
                    report += f"    [INFO] Stream timeline variance verified manually by Engineer.\n"
                else:
                    report += f"    [!!!] DISCREPANCY DETECTED: Stream is shorter than primary record.\n"
    ...
    def ask_ollama(self, url, model, prompt):
        ...
        try:
            # Enforce manual HTTP context deadline to capture system hang early
    ...
    def build_worker_prompt(self):
        ...
        # Build an explicit Markdown Ground Truth Matrix table to anchor the AI context and block hallucinations
        matrix_header = (
            "### CRITICAL GROUND TRUTH DATA MATRIX (PARSED VIA PYTHON)\n"
            "Treat the following metadata fields as absolute factual baseline indicators. "
            "Ignore any conflicting timeline assertions or massive time offsets encountered inside raw stream continuity reports.\n\n"
            "| Stream Filename | True Physical File Duration | Colorspace Profile | Audio Setup |\n"
            "|---|---|---|---|\n"
        )

        for name, data in self.results.items():
            matrix_header += f"| {name} | {data.get('duration_str', 'Unknown')} | {'10-bit HDR' if data['is_hdr'] else '8-bit SDR'} | {data['audio_codec']} {data['audio_channels']} |\n"

        matrix_header += "\n\n"

        system_prompt = (
                "You are an elite Audio/Video Archival Engineer. Your client is building a master "
                "archive for a high-end home theater featuring a true 4K HDR Projector and a premium "
                "KEF 5.1 Surround Sound system.\n\n"
                "YOUR TASK:\n"
                "You must review the provided file metadata and raw FFmpeg demuxer logs to determine the ultimate archive selection.\n\n"
                + matrix_header +
                ...
                "3. INTEGRITY: Penalize streams that crashed with 'Conversion failed!' or required the "
                "'+discardcorrupt' fallback. However, a slightly messy 5.1 HDR stream is better than a "
                "flawless stereo SDR stream, because MKVToolNix will fix the timestamps later.\n"
        )

        if override_dur:
            system_prompt += "4. DURATION: Some streams may have a shorter duration. THE CLIENT HAS EXPLICITLY VERIFIED THAT THE MATCH DATA IS COMPLETE. The extra time on the longer streams is just post-match ads. DO NOT penalize any stream for being shorter.\n\n"
        else:
            system_prompt += "4. DURATION (FATAL): If a stream is missing significant playtime compared to the longest capture, it means the recording cut off before the match ended. You must flag this explicitly in your analysis and DO NOT SELECT IT under any circumstances.\n\n"

        system_prompt += "Here are the truncated FFmpeg logs:\n\n"

        for name, log_content in self.raw_logs.items():
            dur_str = self.results[name].get('duration_str', 'Unknown')
            system_prompt += f"--- LOG FOR STREAM: {name} (Factual Track Length: {dur_str}) ---\n"
        ...
        system_prompt += "Weigh the technical indicators step-by-step, cross-check with the factual matrix above, and explicitly declare a winner."
    ...
    def build_judge_prompt(self, worker_responses):
        system_prompt = (
            ...
            "2. Review the individual Worker verdicts. Pinpoint anomalies where models misread raw log timestamps (e.g., misinterpreting discontinuity offsets as video duration).\n"
            "3. Deliver the final, executive ruling on which file to archive, and justify why your decision "
            "provides the best experience for a 4K HDR, Dolby 5.1 hardware setup.\n\n"
            "### ABSOLUTE STRUCTURAL TRUTH MATRIX:\n"
        )

        for name, data in self.results.items():
            system_prompt += f"- File: {name} | True Calculated Length: {data.get('duration_str', 'Unknown')} | Profile: {'10-bit HDR' if data['is_hdr'] else '8-bit SDR'} | Audio: {data['audio_channels']}\n"

        system_prompt += "\n--- THE AI WORKER VERDICTS ---\n"
        for worker, response in worker_responses.items():
            system_prompt += f"VERDICT FROM {worker}:\n{response}\n\n"

        system_prompt += "Synthesize these findings, correct any worker logical errors using the matrix, and declare the absolute winner."
        return system_prompt
