...
class MasterArchiverApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Golden Standard: Master Agentic Archiver v3 (Dual-Clock Engine)")
    ...
    def orchestrate_analysis(self):
        ...
        if has_duration_warning and not self.var_ignore_dur.get():
            self.root.after(0, lambda: messagebox.showwarning("⚠️ DURATION DISCREPANCY DETECTED",
                                                              "One or more streams are significantly shorter than the primary record.\n\nThe algorithm penalized them. AI agents will now review the corrected Dual-Clock metrics."))
    ...
    def parse_metrics(self, output):
        ...
        # --- THE DUAL-CLOCK ENGINE FIX ---
        # 1. Grab Header Duration (often lies on .ts files)
        ...
        # 2. Grab Physical Processed Time (The last time= flag in the log)
        ...
        if header_sec > 36000:  # Over 10 hours (22h PTS Bug)
            data["duration_sec"] = phys_sec
            data["duration_str"] = phys_str
            data["dur_note"] = f"(PTS Bug Fixed: Header claimed {header_str}, physical time is {phys_str})"
        elif header_sec > 0 and phys_sec > 0 and (header_sec - phys_sec > 300):  # Crashed early
            data["duration_sec"] = phys_sec
            data["duration_str"] = phys_str
            data["dur_note"] = f"(Truncated: Header intended {header_str}, but stream failed/ended at {phys_str})"
        else:
            data["duration_sec"] = phys_sec if phys_sec > 0 else header_sec
            data["duration_str"] = phys_str if phys_sec > 0 else header_str
            data["dur_note"] = "(Clean Full-Length Extraction)"
        # ----------------------------------
    ...
    def generate_verdict(self):
        ...
        for name, data in self.results.items():
            ...
            report += f"[{name}]\n"
            report += f"  - True Extraction Time: {data['duration_str']} {data['dur_note']}\n"
    ...
    def ask_ollama(self, url, model, prompt):
        ...
        try:
            # FIXED: 600 seconds (10 mins) to allow massive 30B models to load into VRAM
    ...
    def build_worker_prompt(self):
        ...
        matrix_header = (
            "### ABSOLUTE GROUND TRUTH MATRIX (CALCULATED VIA DUAL-CLOCK PARSING)\n"
            "This table contains the undeniable physical truth of the files. You MUST use these exact True Duration values. DO NOT guess the duration based on FFmpeg log text, as .ts container metadata is often a hallucination.\n\n"
            "| Filename | True Physical Duration | Note | Profile | Audio |\n"
            "|---|---|---|---|---|\n"
        )

        for name, data in self.results.items():
            matrix_header += f"| {name} | {data.get('duration_str', 'Unknown')} | {data.get('dur_note', '')} | {'10-bit HDR' if data['is_hdr'] else '8-bit SDR'} | {data['audio_codec']} {data['audio_channels']} |\n"
        ...
        system_prompt = (
                ...
                "YOUR TASK:\n"
                "Review the provided Ground Truth Matrix and raw logs to determine the ultimate archive selection.\n\n"
                + matrix_header +
        ...
        if override_dur:
            ...
            system_prompt += "4. DURATION (FATAL): If a stream is missing significant playtime compared to the longest capture in the Ground Truth Matrix, it is fundamentally incomplete. You MUST flag this explicitly and DO NOT SELECT IT under any circumstances.\n\n"
