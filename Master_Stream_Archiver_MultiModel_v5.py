...
class MasterArchiverApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Golden Standard: Master Agentic Archiver v5 (God Mode)")
    ...
    def orchestrate_analysis(self):
        ...
        if has_duration_warning and not self.var_ignore_dur.get():
            self.root.after(0, lambda: messagebox.showwarning("⚠️ DURATION DISCREPANCY DETECTED",
                                                              "One or more streams are significantly shorter than the primary record.\n\nThe algorithm penalized them. AI agents will now review the corrected Triple-Clock metrics."))
    ...
    def probe_file_stats(self, filepath):
        """Uses ffprobe to extract deep JSON metadata from a video file."""
        ...
        try:
            ...
            result = subprocess.run(cmd, capture_output=True, text=True, timeout=20, creationflags=creationflags)
            ...
            return {
                ...
                "size_mb": round(int(format_data.get('size', 0)) / (1024 * 1024), 2),
    ...
    def execute_ffmpeg_pass(self, cmd, log_file, txt_log):
        """Anti-Choke Buffer System. Prevents massive logs from freezing Tkinter UI."""
        output_capture = []
        try:
            creationflags = 0x08000000 if os.name == 'nt' else 0
            process = subprocess.Popen(cmd, stderr=subprocess.PIPE, stdout=subprocess.PIPE, text=True, encoding='utf-8',
                                       errors='replace', bufsize=10 ** 7, creationflags=creationflags)
            pid = process.pid
            ...
            with open(log_file, 'w', encoding='utf-8') as f_log:
                for line in process.stderr:
                    ...
                    # Update UI only 5 times a second to prevent freezing
    ...
    def append_log(self, text_widget, batch_str):
        """Limits the UI text box to 1500 lines so memory never overflows"""
    ...
    def analyze_file_realtime(self, filepath):
        ...
        # 1. Grab Deep Stats of original lying TS file
        ...
        # 3. PROBE THE MKV BEFORE DELETION (The Absolute Truth)
    ...
    def parse_metrics(self, output, mkv_dur_sec):
        ...
        # Grab Physical FFmpeg Processed Time
        ...
        # TRIPLE-CLOCK CONSOLIDATION: MKV IS KING.
        if mkv_dur_sec > 0:
    ...
    def generate_verdict(self):
        ...
        for name, data in self.results.items():
            ...
            # Use deep probe data if available for algorithmic reporting
    ...
    def ask_ollama(self, url, model, prompt):
        ...
        try:
            # 600s TIMEOUT - Crucial for 30B models
