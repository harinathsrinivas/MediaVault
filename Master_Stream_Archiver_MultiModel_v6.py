...
class MasterArchiverApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Golden Standard: Master Agentic Archiver v6 (God Mode)")
    ...
    def execute_ffmpeg_pass(self, cmd, log_file, txt_log):
        ...
        try:
            creationflags = 0x08000000 if os.name == 'nt' else 0
            # Remove text=True to handle raw bytes manually and bypass decoding crashes
    ...
    def ask_ollama(self, url, model, prompt):
        ...
        try:
            # Enforce 600s TIMEOUT - Crucial for local 30B models processing full context
