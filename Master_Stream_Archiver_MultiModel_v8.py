...
class MasterArchiverApp:
    ...
    def build_worker_prompt(self):
        ...
        system_prompt = (
                ...
                + matrix_header +
                "YOUR SCORING DIRECTIVES (YOU MUST EXECUTE THESE IN EXACT ORDER):\n\n"
        )

        if override_dur:
            system_prompt += "STEP 1: DURATION GATE: THE CLIENT HAS VERIFIED ALL STREAMS ARE COMPLETE. Any duration variance is just post-match ads. Proceed directly to Step 2.\n\n"
        else:
            system_prompt += "STEP 1: DURATION GATE (FATAL & UNFORGIVING):\nLook at the 'MKV Truth Duration'. If a stream is missing significant playtime compared to the longest stream in the matrix (e.g., it is 45 minutes while the longest is 2 hours), it is fundamentally broken. YOU MUST IMMEDIATELY DISQUALIFY IT. You are strictly forbidden from choosing an incomplete file, no matter how good its audio or visuals are.\n\n"

        system_prompt += (
            "STEP 2: AUDIO & VISUAL EVALUATION (For surviving streams only):\n"
            "Among the streams that passed Step 1, prioritize the following:\n"
            "- VISUALS: True 10-bit HDR (BT.2020 / SMPTE2084) and 59.94 FPS.\n"
            "- AUDIO (CRITICAL): Discrete AC-3 Dolby Digital 5.1 over flat AAC Stereo.\n"
            "- INTEGRITY: A slightly messy 5.1 HDR stream is better than a flawless stereo SDR stream, BUT ONLY IF BOTH PASSED STEP 1.\n\n"
        )
