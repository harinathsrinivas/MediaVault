...
## IMP-E16 step 1 — store TMDB synopsis (title + per-episode) + expose in /api/items — [status: done]
- Executor: executor-opus
- Model: opus
- Mode: single-executor
- Branch: `feature/imp_e16_ui_wow` (work done in the main working dir; this run is on `feature/imp_e3_u3_d17_tmdb_posters_rename` per gitStatus — orchestrator handles branching/commits).
- Files changed: `main.py`, `tests/test_enrich_metadata.py`, `tests/test_web_items.py`, `tests/test_web_tree.py`.
- Outcome: `cmd_enrich_metadata --apply` now persists the TMDB synopsis. (1) The step-1 metadata loop writes `metadata.overview` = `res["overview"]` (the movie/show synopsis) onto every unit id — covers movies fully and SEEDS season_maps + episode leaves with the show synopsis (written only when TMDB has one, so a no-overview match leaves metadata untouched). (2) A new `_apply_episode_overviews(unit, series_id, api_key)` refines SHOWS: ONE cached `GET /3/tv/{id}/season/{n}` (SEASON DETAILS) per season yields `episodes[]`, from which each episode leaf gets `metadata.overview` (episode synopsis) + `metadata.episode_title` (episode `name`, e.g. "Pilot"). Keyed by the same `_episode_se_of` (season, episode) the still loop uses. (3) `items_payload()` now emits, per row: `overview` (metadata.overview or null), `episode_title` (metadata.episode_title or null), and `backdrop_available` (bool — a LIVE `resolve_artwork_path(library, mid, "fanart")` check, gated on `has_anchor` + short-circuited exactly like `poster_available`). `build_tree()` propagates all three into tree leaf nodes automatically (it does `dict(leaf)`), so the folder view gets them for free. All 479 Python tests pass; smoke green.
- Key decisions:
  - **Additive season-details fetch, NOT a refactor of the image calls.** The plan offered replacing the per-season `/season/{n}/images` + per-episode `/season/{s}/episode/{e}/images` calls with the single season-details call (which carries season `poster_path` + per-episode `still_path`). I deliberately did NOT — the season-details `still_path` is a single primary still, whereas the existing path fetches a stills LIST and `_pick_still_path` selects the HIGHEST-vote still (asserted by `test_apply_picks_highest_vote_still`), and the existing per-episode LOCAL-wins / error / empty fall-backs are individually tested. Replacing them would regress proven image behavior and force rewriting ≥3 image tests. So I ADDED exactly one cached `GET /tv/{id}/season/{n}` per season for overview+name only; every existing image call is byte-for-byte unchanged. Net new API cost: one cached call per season per show (idempotent via the on-disk TMDB cache).
  - **Overview seed-then-refine (graceful degradation).** Step 1 writes the SHOW overview to all unit ids; step 3b overrides episode leaves with their own episode overview. Consequence: if a season's season-details call FAILS, that episode keeps the show synopsis (degrades to something useful) rather than nothing, and gets no `episode_title`. Tested (`test_apply_show_season_details_failure_degrades_to_show_overview`).
  - **Fields written only when truthy.** `overview`/`episode_title` are written only when TMDB returns a non-empty value, keeping `metadata` clean (the smoke "SMK" canned result has no overview → no `overview` key churn → smoke stays green with no conftest change). The shared `MockTMDB` catch-all returns `{}` for the new season-details URL, which `_season_episode_meta` treats as "no episodes" — so the smoke show-apply path degrades cleanly without touching `tests/conftest.py`.
  - **`ENTRY_TYPE_KEYS` NOT changed.** `overview`/`episode_title` live INSIDE the `metadata` sub-dict (alongside the existing `title`/`year`/`tmdb_id`), NOT as new TOP-LEVEL entry keys. `ENTRY_TYPE_KEYS` + its guard test only track top-level keys (`folder_path`/`filename`/`status`/`children`/`alias_of`/`parent_id`); `metadata` is already a documented optional leaf key. So no registry change and no new iterator hazard — verified `test_entry_schema_guard.py` still green.
  - **Did NOT touch `webui/static`/`server.py`.** The front-end consumes these fields in a later step; `/api/items` returns the payload verbatim, so the new fields reach the SPA with no server change.
  - **Backfill note:** a one-time `enrich_metadata --apply` re-run BACKFILLS overviews onto the already-enriched library — it is idempotent and images are already local (LOCAL-wins skips all downloads), so the re-run only adds the JSON `overview`/`episode_title` fields plus the new cached season-details GET per season.
- Verification:
  - `.venv\Scripts\python.exe -m pytest tests/test_enrich_metadata.py tests/test_web_items.py -q` → **52 passed, 1 warning** (pre-existing Starlette/httpx TestClient deprecation; unrelated).
  - `.venv\Scripts\python.exe -m pytest tests/smoke -q` → **69 passed, 1 warning** (MANDATORY smoke-gate — main.py touched).
  - `.venv\Scripts\python.exe -m pytest tests/test_entry_schema_guard.py tests/test_web_media_image.py tests/test_web_tree.py -q` → **55 passed** (regression check: `build_tree` carries the new leaf fields — updated `_LEAF_BASE_KEYS` in `test_web_tree.py`; entry-schema guard unaffected).
  - `.venv\Scripts\python.exe -m pytest -q` (full suite) → **479 passed, 1 warning**.

## IMP-E16 — "refresh online metadata" backend: OMDb ratings/awards/box-office + mvonline.json cache + /api/detail merge — [status: done]
- Executor: executor-opus
- Model: opus
- Mode: single-executor
- Branch: `feature/imp_e16_ui_wow` (work done in the main working dir; the orchestrator handles branching/commits — gitStatus shows `feature/imp_e3_u3_d17_tmdb_posters_rename` at dispatch, but this is the IMP-E16 OMDb backend).
- Files changed: `mvcommon.py`, `main.py`, `webui/server.py` (no edit needed — see Key decisions), `.gitignore`, `mvconfig.example.json`, `tests/conftest.py`, `tests/test_web_detail.py`, `tests/test_refresh_online.py` (new).
- Outcome: Built the backend for the hover-dossier's online ratings. (1) **mvcommon getters** `omdb_api_key()` / `rapidapi_imdb_key()` / `rapidapi_rt_key()` — runtime getters (no import-time binding) reading `omdb.api_key` / `rapidapi.imdb_key` / `rapidapi.rt_key`, "" when absent, via a shared `_config_string(section,key)` mirroring `tmdb_api_key()`. (2) **Online-metadata cache** in main.py: `ONLINE_CACHE_PATH` (gitignored `mvonline.json` at the repo root, keyed by `str(tmdb_id)`), `online_cache_load/get/set` (atomic tempfile+os.replace, the token-store idiom), plus `_online_entry_is_fresh` (≤ `ONLINE_FRESH_DAYS`=14, via `mvcommon._parse_iso`/`_now_utc`). (3) **OMDb client** `omdb_fetch(imdb_id=None,title=None,year=None)` → `{imdb_id, ratings:{imdb,rotten_tomatoes,metacritic}, rated, runtime, awards, boxoffice}` or None — by `?i=` (preferred) or `?t=&y=`; `_omdb_get` caches raw responses under `OMDB_CACHE_DIR` (sibling of TMDB_CACHE_DIR) and degrades to None on network/non-200/bad-JSON/`Response:"False"`, never raising; `_omdb_parse_ratings` maps the 3 Source names and normalizes IMDb `8.8/10`→`8.8`, MC `74/100`→`74`, RT keeps `87%`; `N/A` fields drop to "". (4) **`cmd_refresh_online([id_or_prefix] [--force] [--library X])`** — reuses `_gather_enrich_units` (already skips `multi_ep_alias`, groups shows, honors id/library filters), reads each unit's stored `metadata.tmdb_id` via `_unit_preset_tmdb_id`, **dedupes to distinct tmdb_ids** (a show's episodes → ONE OMDb fetch; episodes inherit the show's ratings), skips entries fresh within 14 days unless `--force`, resolves each imdb_id via TMDB (`_resolve_imdb_id`: movie `/3/movie/{id}.imdb_id`, tv `/3/tv/{id}/external_ids.imdb_id`), calls `omdb_fetch`, stamps `fetched_at`, and `online_cache_set`s — printing live per-title progress (`[i/n] Title (tmdb) -> IMDb 8.8 · RT 87% · MC 74`) + a final `fetched/cached-skip/no-imdb/failed/no-tmdb` summary. **NEVER mutates library_*.json** (writes only mvonline.json + the on-disk caches). Argv arm `refresh_online` added + a help line. (5) **`/api/detail` merge**: `tmdb_detail` now calls `_merge_online_metadata(out, tmdb_id)` before returning — CACHE-ONLY (no live OMDb call, so the dossier stays fast), adding `ratings`/`rated`/`awards`/`boxoffice` only when present; for an episode `out["tmdb_id"]` is already the SHOW id (enrich stamps it on every leaf) so episodes inherit the show's ratings. The merge is independent of the TMDB key (a populated cache enriches the dossier even with no TMDB key).
- Key decisions:
  - **Cache helpers + merge live in `main.py`, NOT `mvcommon.py`.** The task allowed either. Both consumers (`tmdb_detail`, `cmd_refresh_online`) are in main.py, so a single module-level `main.ONLINE_CACHE_PATH` is the one binding both read and tests monkeypatch — avoiding a cross-module binding hazard (a `from mvcommon import ONLINE_CACHE_PATH` in main would create a second stale binding). `webui/server.py` needed NO edit: `GET /api/detail` already returns `tmdb_detail(...)` verbatim, so the merged fields flow through unchanged (surgical-change discipline — listed as touchable but no change was warranted).
  - **`tmdb_detail` restructured to a single return so the merge runs on BOTH paths.** The no-TMDB-key early-return was changed from `if not api_key: return out` to `if api_key: <fetch>` falling through to one `_merge_online_metadata` + `return`. This guarantees the cache merge applies even when no TMDB key is configured (the cache is independent of TMDB). Verified by `test_detail_merge_with_no_tmdb_key`.
  - **Ratings normalized to the compact contract shape** (`{"imdb":"8.8","metacritic":"74","rotten_tomatoes":"87%"}`) — the `/N` denominator is stripped for the score sources (IMDb x/10, MC x/100); RT's `%` is kept. This matches the task's example cache entry AND the example progress line (`IMDb 8.8 · RT 87% · MC 74`). The frontend (a later step) consumes this shape.
  - **`runtime` is NOT merged into the dossier** even though OMDb returns it. The task's merge list is exactly `ratings`/`rated`/`awards`/`boxoffice`; TMDB already supplies a numeric `runtime` and overwriting it with OMDb's `"148 min"` string would change a typed field's shape. `omdb_fetch` still returns `runtime` (stored in the cache) for any future consumer; the dossier just doesn't surface it.
  - **`--force` re-derives mvonline.json but the OMDb response cache still serves the raw payload.** `--force` bypasses the 14-day freshness check on mvonline.json; `_omdb_get`'s on-disk cache makes the re-derive cheap (no second network hit). This is the documented "cache the raw response so re-runs are cheap" intent — OMDb data rarely changes within a session.
  - **`ENTRY_TYPE_KEYS` NOT changed.** The online cache is a SEPARATE file (mvonline.json) keyed by tmdb_id; it adds no library entry type and no shared library field. `cmd_refresh_online` is a whole-library iterator but is alias/season_map-safe FOR FREE because it reuses `_gather_enrich_units` (which already skips `multi_ep_alias` and groups by show). `test_entry_schema_guard.py` stays green.
  - **conftest `sandbox` now also redirects `ONLINE_CACHE_PATH` + `OMDB_CACHE_DIR`** to the sandbox tree (with a hard-guard against the real repo-root cache), so NO test can read/write the real mvonline.json once a real `refresh_online` run creates one — closing a test-pollution hole (tmdb_detail merges this cache, so an un-redirected real file would silently leak ratings into every detail test). The per-test `online_cache` fixture (in both test files) overrides it to a path it seeds; it is requested AFTER `sandbox` in every consumer's signature so its monkeypatch wins.
- Key decisions another step may need:
  - The cache contract is `{"<tmdb_id>": {"imdb_id","ratings":{imdb,rotten_tomatoes,metacritic},"rated","runtime","awards","boxoffice","fetched_at"}}`. The frontend dossier step reads `ratings`/`rated`/`awards`/`boxoffice` off `/api/detail` (present only when cached).
  - Populate the cache with `python main.py refresh_online` (needs BOTH `omdb.api_key` AND `tmdb.api_key` in mvconfig.json — TMDB resolves each title's imdb_id). The real mvconfig.json already carries an `omdb.api_key`.
- Verification:
  - `.venv\Scripts\python.exe -m pytest tests/test_refresh_online.py tests/test_web_detail.py -q` → **44 passed, 1 warning** (pre-existing Starlette/httpx TestClient deprecation; unrelated). [REQUIRED acceptance command]
  - `.venv\Scripts\python.exe -m pytest tests/smoke -q` → **69 passed, 1 warning** (MANDATORY smoke-gate — main.py + mvcommon.py touched). [REQUIRED acceptance command]
  - `.venv\Scripts\python.exe -m pytest tests/test_entry_schema_guard.py tests/test_mvcommon.py -q` → **14 passed** (registry + getters regression).
  - `.venv\Scripts\python.exe -m pytest -q --deselect "tests/test_web_tree.py::test_tree_leaf_node_keys_and_type"` (full suite) → **526 passed, 1 deselected, 1 warning**.
  - **Pre-existing unrelated failure noted:** `tests/test_web_tree.py::test_tree_leaf_node_keys_and_type` fails on the CURRENT tree EVEN WITH my changes stashed (proven via `git stash` + run) — its `_LEAF_BASE_KEYS` is stale vs. leaf entries that gained `tech`/`actual_size_bytes`/`release_name` in a change AFTER the prior IMP-E16 step (which reported 479 passing). It is NOT in my acceptance set and is untouched by this OMDb work; flagged here for the orchestrator (a one-line `_LEAF_BASE_KEYS` update is the fix, owned by whoever introduced those leaf keys).
