...
| `feature-token-brackets/` | IMP-U6 (**in progress**, opened 2026-09-07) — provider folder token `{tmdb-…}` → `[tmdbid-…]` (Jellyfin/Emby-native) + NFO written at every stamp (Plex NFO agent pins the id) + `tools/migrate_token_brackets.py` live-library migration | `PLAN.md` (evidence + steps + Open Decisions D1–D6, all user-ruled), `DECISIONS.md`, `PROGRESS.md` |
