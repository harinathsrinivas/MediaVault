...
- [x] 4. [model: sonnet] ADB push retry protocol tests in `tests/test_cmd_push_retry.py`.
...
- [x] 5. [model: opus] Add the `mock_fetch` fixture to `tests/conftest.py`.
...
- [x] 6. [model: sonnet] Wrap the inner body of `trigger_download` with retry-once-after-5s on both 0-thumbnails and exceptions, with a per-attempt print.
  - Files: `mainfetch.py`
...
- [x] 7. [model: sonnet] trigger_download retry tests in `tests/test_trigger_download_retry.py`.
...
- [x] 8. [model: haiku] Mark IMP-C2 done and document the retry wrapper.
...
- [x] 9. [model: haiku] Fill the Completion report and sync the root PLAN.md.
