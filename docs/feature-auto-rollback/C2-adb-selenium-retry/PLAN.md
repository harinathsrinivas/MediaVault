...
- [ ] 2. [model: sonnet] Unit-test `retry()` in `tests/test_mvcommon.py`.
...
- [ ] 3. [model: opus] Wrap `adb push` + atomic `mv` in `cmd_push` with `retry()`, including pre-retry `.partial` cleanup and a per-attempt print.
  - Files: `main.py`
