...
- [ ] 1.0. [model: haiku] [effort: low] Seed the tracked plan/decisions artifacts for this feature.
...
- [ ] 1.1. [model: opus] [effort: high] [candidates: 3] Add a read-only `items_payload()` data builder in `main.py` (library-centric, season-aware, alias/season_map-safe) returning every physical leaf with category + state + poster/tmdb facts.
...
- [ ] 1.2. [model: sonnet] [effort: medium] Expose `GET /api/items` in `server.py`.
...
- [ ] 1.3. [model: opus] [effort: high] [candidates: 2] Add the top-level media-type tab bar + per-tab disk-state sub-view grouping to the SPA, driven by `/api/items`.
...
- [ ] 2.1. [model: opus] [effort: high] Add `fetch_restore` (and optional advanced `fetch`) to `ACTION_TABLE`, wrapping the existing `cmd_fetch_restore`.
...
- [ ] 2.2. [model: opus] [effort: max] [candidates: 3] Make the job worker flush captured stdout INCREMENTALLY and expose a parsed `progress {done,total}` derived from chunks-done/total_chunks.
...
- [ ] 2.3. [model: opus] [effort: high] [candidates: 2] Wire the Archived sub-view: Fetch & Restore button → poll job → growing chunk-% border → auto-flip the card to Fetched·not-archived on done.
...
- [ ] 2.4. [model: opus] [effort: high] Tests for the progress flush + fetch_restore action (serialized-worker safe, alias-safe).
...
- [ ] 2.5. [model: sonnet] [effort: medium] Extend the smoke suite to drive `fetch_restore` via the web action path + assert `/api/items` import-safety.
...
- [ ] 2.6. [model: sonnet] [effort: medium] Architect + docs for the fetch-in-UI behavior change.
...
- [ ] 4.5. [model: opus] [effort: high] Tests for the token auth (localhost-exempt; remote-required; read-flag behavior).
...
- [ ] 5.1. [model: opus] [effort: high] Add `tmdb_id` to the leaf entry metadata schema + a manual `set_tmdb` override command; update `ENTRY_TYPE_KEYS`/guard if a shared field set is touched.
...
- [ ] 5.3. [model: opus] [effort: max] Implement the crash-safe cascading `rename_folder` CLI command (IMP-D17): rename the on-disk folder + atomically rewrite `folder_path` for every descendant entry; works on archived dummies; no rehash.
...
- [ ] 5.7. [model: opus] [effort: high] Wire posters + tmdb into `/api/items` + the SPA cards (real poster grid in Archived + other sub-views).
