...
0. **IMP-C22** — anime per-episode enrichment mis-parse (see pointer above); do this first.
...
**Headline NEXT: IMP-C22 (anime per-episode enrichment fix), then IMP-S1** (zero code, do anytime — Jellyfin is the couch surface the vault is being built for).
...
**Two open items: IMP-R10** (change-gated) and **IMP-C22** (pending, not gated). R10's fix is deferred on a user decision (2026-08-24, "document more - can fix later"). C22 (registered 2026-08-29) is documented in full and ready to implement — no change-gate blocks it, only implementation capacity. **IMP-C23 done** (2026-08-31).
