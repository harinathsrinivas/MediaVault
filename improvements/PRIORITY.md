...
>
> **Last updated:** 2026-09-23 (**IMP-U7 + IMP-U8 registered; IMP-U6 done and merged**. **IMP-U7 is the new `👉 SUGGESTED NEXT TASK`** — a fifth instance of the drift-between-duplicated-parsers class, and the first where both copies live in the same file: `_show_folder_of` recognises a season folder as `^season\s*\d+$`/`^s\d+$` while `normalize_season_folders` *builds* them as `<Show> Season NN (year)`, so the builder's own output can never satisfy the recogniser. A single-season unit then resolves the SEASON folder as the show folder and `enrich_metadata` stamps the show token onto it — silently undoing the IMP-U6 normalization on a measured **47 season folders across 11 shows**. It only bites shows whose seasons span multiple years, because library ids embed the season's air year; Silicon Valley (all seasons under one id year) groups correctly and is unaffected, which is why the bug is invisible on some shows. **IMP-U8** records that `enrich_metadata` can correct neither a wrong token nor a wrong title, and that no `set_title` exists — two real movies displayed as `Chainsaw Scumfuck` and `En Mis Zapatos` on Emby/Jellyfin because the servers mirror MediaVault's library faithfully. **IMP-U6's canonical format was corrected from `[tmdbid-<id>]` to `{tmdb-<id>}`** (decision D12) after a 20-folder matrix on real Plex/Emby/Jellyfin installs proved `[tmdbid-…]` is invisible to Plex — Plex rejects the `id` SUFFIX and `=` separator, not brackets, so the widely-repeated "Plex ignores square brackets" claim this task had adopted was false; dual-token was also disproved (Jellyfin deletes such a folder from the library). Shipped via PR #53/#54/#55; `docs/FOLDER_NAMING_CONVENTIONS.md` is the reference. Earlier: **IMP-U6 registered**: canonical `[tmdbid-<id>]`
...
**Headline NEXT: 👉 IMP-U7** (season-folder name drift — `enrich_metadata` undoes the IMP-U6
normalization on 47 season folders; small, not gated, actionable today). Then **IMP-C24** (concurrent
library writes — needs a change-gate ruling), then **IMP-C22**, **IMP-U8** and **IMP-D23**.
...
**Five open items: IMP-U7** (new 2026-09-23 — highest urgency, not gated), **IMP-R10** (change-gated,
deferred), **IMP-C24** (change-gated, awaiting ruling), **IMP-C22** (pending, not gated) and **IMP-U8**
(new 2026-09-23, not gated). **IMP-U7 outranks everything else in this band**: it is not a latent
exposure but an *active* regression on the happy path — running `enrich_metadata` on any multi-year
show today silently re-stamps season tokens and reverses the completed IMP-U6 migration. R10's fix is deferred on a user decision (2026-08-24,
...
|---|---|---|---|---|
| 1 | 🔴 **IMP-U7** | `_show_folder_of` (`main.py` ~2151) recognises a season folder as `^season\s*\d+$`/`^s\d+$`, but `normalize_season_folders` (~5003) *builds* them as `<Show> Season NN (year)` — the builder's output can never match the recogniser. A single-season unit therefore resolves the SEASON folder as the show folder and `enrich_metadata` stamps the show's token onto it, undoing IMP-U6's normalization and re-introducing exactly the season-level ids `FOLDER_NAMING_CONVENTIONS.md` §3 forbids. Dry-run measured **47 season folders across 11 shows**. Fifth instance of the drift-between-duplicated-parsers class (C18/C22/C23/U6) — and the first where both copies are in the same file | low — pure path logic; no rollback/PONR/`ENTRY_TYPE_KEYS` involvement. The regression pin (builder output must satisfy recogniser) matters more than the fix | not change-gated — ready to implement; see `improvements_tierU.md` IMP-U7 |
| 2 | 🚦 **IMP-C24** | `mvcommon.load_library`/`save_library` have no lock; two concurrent mutating commands (any two of prep/push/replace/restore/`web`) each hold a stale in-memory snapshot across a slow operation and the later save silently erases the earlier one's change — blast radius is all four library JSONs, not one. Already caused a real incident: 13 corrupted entries + one dummy uploaded to Google Photos in place of a real episode | **gated** — the recommended fix touches the wrapping of `cmd_prep`/`cmd_push`/`cmd_replace`/`cmd_restore` and `RollbackJournal`'s own `save_library()` call | 🚦 **needs a user decision** (`CLAUDE.md` change-gate); options + recommendation in `docs/feature-library-concurrency/PLAN.md` |
| 3 | 🚦 **IMP-R10** | a transient lock on `.mediavault_txn.json` during `cmd_replace`'s PONR write is caught by the retry loop meant for a locked *media* file → spurious `IRREVERSIBLE`, and the handler advises `fetch_restore` (a 62 GB re-download) while the master sits on local disk as `.tobedeleted` | **gated** — touches `mark_point_of_no_return()` placement | 🚦 **needs a user decision** (`CLAUDE.md` change-gate); documented in `docs/edge-case-replace-ponr-journal-lock/` |
...
| 4 | 🟠 **IMP-U8** | `enrich_metadata` can correct neither a wrong token (the stamp is skipped whenever *any* token is present) nor a wrong title (only id-shaped titles are overwritten), and there is no `set_title`. A wrong id/title in the library propagates to Plex/Emby/Jellyfin, which mirror it faithfully — two real movies displayed as `Chainsaw Scumfuck` and `En Mis Zapatos` until repaired by hand-editing a library JSON | low — the repoint path stays opt-in so the default double-stamp guard is unchanged | not change-gated — see `improvements_tierU.md` IMP-U8 |
