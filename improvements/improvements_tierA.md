...
- **Minimal slice shipped via IMP-E15 (2026-06-23):** `mvcommon.py` now loads `mvconfig.json` at startup with a minimal schema: `web.host`, `web.port`, `tmdb.api_key`. (Note: `web.token` was removed in the IMP-E15 auth redesign — auth is now admin-minted, expiring tokens in the separate `mvtokens.json`.) An absent or malformed file falls back to today's defaults (no behavioral change for existing workflows). `mvconfig.json` is gitignored; `mvconfig.example.json` is checked in. The full migration of all constants (`LIBRARY_*`, `LOCAL_ROOT`, `MKVMERGE_PATH`, `CHROME_PROFILES`, `DEVICE_ALIASES`, etc.) remains pending under this task.
- Status: pending (minimal slice in `feature/imp_e15_mobile_tailscale_auth`; full migration pending)
