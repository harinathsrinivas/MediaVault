...
## IMP-D23: prep re-hashes an already-prepped `local_ready` entry — a failed-push resume is enormously expensive

- Category: other
- Priority: high
- Files: `main.py` — `cmd_prep`'s early-skip block (~1049-1080); new `cmd_push_rep`/`cmd_push_rep_season` adjacent to `cmd_prep_push_rep`/`cmd_prep_push_rep_season` (~7298/~7355); the CLI dispatcher (`elif cmd == "prep_push_rep":` block, ~9990-10046, is the pattern to mirror for the new commands' argv parsing). Full options/steps/recommendation: `docs/feature-library-concurrency/PLAN.md`.
- Current behavior: `cmd_prep`'s early-skip (main.py:1056) only fires when `entry.get("uploaded")` is truthy or `status` is in `("onboarded", "archived", "restored_local")`. An entry left at `status="local_ready"`, `uploaded=False` — **exactly the state a successful prep + a failed push leaves behind** — falls through and re-runs the full pipeline: a fresh whole-file SHA256 (`calculate_file_hash`) plus a MediaInfo deep scan, even though the stored `hash` is already correct and the `uid`/`<short_id>.sha256` sidecars already exist on disk. A secondary safety net just below only catches dummy-sized files (`if os.path.getsize(filepath) < DUMMY_MAX_BYTES: skip`) — it does nothing for a genuine, already-hashed real file. For a 75.59 GB file (verified: `tech_spec.size_bytes = 81165098722`) this is many minutes of pure waste on every resume attempt; for a 13-episode UHD season it multiplies across every episode. The entry schema has no `mtime` field today (only `hash` and `tech_spec.size_bytes`), so there is no cheap "did this file change since prep?" check available without adding one.
- Proposed change (direction; options evaluated with a recommendation in `docs/feature-library-concurrency/PLAN.md`): (a) new `push_rep`/`push_rep_season` commands taking the same args as the `prep_push_rep*` family minus the prep leg — additive, wraps `cmd_push`+`cmd_replace` exactly like `cmd_prep_push_rep` already wraps all three, no rollback-contract change; (b) an opt-in `--assume-unchanged` flag on `cmd_prep` that extends its EXISTING early-skip pattern with a size-match heuristic (same `RollbackJournal`-free, zero-artifact contract as the two checks already there) — honestly a heuristic, not proof, so opt-in only, never the default; also bank `tech_spec.local_mtime` at prep time (additive, optional field) to strengthen a future check; (c) a `--skip-prep`/`--resume` flag that edits the printed season resume-range message itself — deferred, because that message is explicitly part of the change-gated auto-rollback contract (`CLAUDE.md`) and would need its own separate sign-off, mirroring how IMP-D20's `--extras` addition to that same message required one. The plan's recommendation is (a) as the primary fix (fully solves the incident's actual resume scenario, touches no change-gate) plus (b) as a complementary opt-in safety net for users who keep using the original `prep_push_rep_season` command name; (c) deferred.
- Rationale: this is the exact gap the user was manually working around — via a risky parallel `push_group` + `replace` workaround — when they triggered the IMP-C24 incident. A cheap, safe resume path removes the incentive for that workaround entirely.
- Goal: resuming a failed push after a successful prep costs zero re-hashing, whether via a new dedicated command or an explicit opt-in flag — never a silent, unproven skip by default.
- Effort estimate: small-medium (option (a) is a small additive wrapper; option (b) touches the entry schema and needs the schema-guard round-trip test).
- Risk: low for (a) — pure additive command reusing unmodified `cmd_push`/`cmd_replace`. Low-medium for (b) — the size-match heuristic is honestly imperfect (a same-size in-place edit would silently reuse a stale hash); mitigated by keeping it strictly opt-in and documenting the limitation, matching the project's standing "no automatic quality/trust decisions" stance. Any new prep skip path must preserve the existing early-skip contract exactly: zero artifacts created, no `RollbackJournal` opened (main.py:1057-1058).
