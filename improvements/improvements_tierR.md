...
## IMP-R10: a lock on the journal during `cmd_replace`'s PONR write is misread as a locked media file

- Category: bug
- Priority: high (Band 0 — reports IRREVERSIBLE on a fully recoverable state, and prints advice that costs a 62 GB re-download)
- Files: `main.py` — `cmd_replace`'s commit retry loop, `RollbackJournal.mark_point_of_no_return()` / `_flush()`, the post-PONR handler that raises `RollbackHardFail` · found 2026-08-24 archiving `mov-kor-2003-ataleoftwosisters`
- Current behavior / WHEN IT OCCURS: `mark_point_of_no_return()` sets `self.crossed_ponr = True` and *then* calls `_flush()`. That flush is a write-temp → `fsync` → `os.replace`, and it sits INSIDE the same `try` as the master rename, whose `except PermissionError` exists to retry a locked **media file**. Observed sequence: attempt 1 renamed the master to `.tobedeleted` successfully, then `_flush()`'s `os.replace` hit a transient lock on `.mediavault_txn.json` (Windows Defender / Search Indexer, right after ~62 GB of writes into the folder); the `PermissionError` was caught, printed `⚠️ File busy or locked. Retrying... (1/3)`, and looped; attempt 2's `os.chmod(original, ...)` raised `WinError 2` because the rename had already moved it; that fell to `except Exception`, which called `journal.rollback(library)` under a comment asserting "still pre-PONR — reversible"; the in-memory flag said otherwise, so `rollback()` raised `RuntimeError`, wrapped by the outer handler into `RollbackHardFail` → `❌ IRREVERSIBLE: replace failed after the commit point`. **Smoking gun:** the folder was left with `.mediavault_txn.json` (276 B, `"crossed_ponr": false`) beside an orphan `.mediavault_txn.json.tmp` (275 B, `"crossed_ponr": true`) — one byte apart, exactly `true` vs `false`.
- **Nothing was lost, and the failure message was actively harmful.** The two-rename pattern kept the master safe at `<original>.tobedeleted`; all 10 chunks were already pushed; the entry had `uploaded: True`. But the handler printed `To recover the file from the cloud: fetch_restore <id>` — which would have pulled 62.6 GB back down over Selenium to recover a file sitting one filename away on local disk. The adjacent line correctly says the C9 stale sweep recovers it next run, so the two lines contradict each other.
- Recovery that worked (no file surgery): `recover <id>` → delete the orphan `.mediavault_txn.json.tmp` → `replace <id>` (C9 stale sweep restores the master and aborts by design) → `replace <id>` again (clean). Note this succeeded *because* the durable write failed: `recover_journal()` refuses a journal whose on-disk `crossed_ponr` is true, so the mirror-image case (flush succeeds, something later fails) recovers by a different route.
- Proposed change — **NOT decided**, three candidate shapes: (a) narrow the `try` so only `os.rename` is inside the retrying `except PermissionError`; (b) call `mark_point_of_no_return()` after the retry loop, once the rename is known to have succeeded; (c) treat a flush failure *after* a successful rename as non-fatal — the rename is the real commit, the journal is a record of it. (c) changes durability semantics and is the most gated. Orthogonal to all three: the handler should check for `<original>.tobedeleted` before advising `fetch_restore`, and `File busy or locked` / `Error removing file` both name the wrong operation.
- Open question for the rollback owner: should failing to *persist* the PONR marker be fatal at all, when the rename it marks already committed?
- Rationale: the lock lands on a small JSON file in a folder that just absorbed tens of GB, so the bug is **more likely on large slow archives** — exactly the runs where a false IRREVERSIBLE banner is most alarming and the wrong `fetch_restore` advice costs the most. `mvcommon.retry()` (IMP-C2) was never applied to this hand-rolled loop; the Tier C header already records that it predates it.
- Goal: a transient journal-write lock never reports an irreversible failure, and no failure path recommends `fetch_restore` while the master is still on local disk.
- Effort estimate: small (the code), but the decision is the work.
- Risk: **change-gated** — touches `mark_point_of_no_return()` placement, explicitly listed in `CLAUDE.md` and `docs/feature-auto-rollback/ROLLBACK_MECHANISM.md` §10.
- If skipped: recurs on every large archive whenever AV/indexer timing lines up; the operator sees IRREVERSIBLE, is told to re-download 60+ GB, and may do it — while the file sits next to the dummy.
- Status: pending — documented, not fixed (user decision 2026-08-24: "document more - can fix later"). Full incident, state matrix, recognition checklist and acceptance criteria: `docs/edge-case-replace-ponr-journal-lock/README.md`.
- **Change-gate:** alters `mark_point_of_no_return()` placement / PONR semantics — ask first, and state exactly what differs from the documented behavior.
