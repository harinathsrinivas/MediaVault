...
## IMP-U6: Provider folder token `{tmdb-…}` → `[tmdbid-…]` + Plex NFO id-pinning

- Category: UX / metadata (library beauty)
- Priority: medium
- Files: `main.py` (token predicate family ~1687/9585, enrich stamp sites ~2635/2689/7730, `_ancestor_show_folder_image` ~9665, `suggest_target_folder` ~8363, help text), `webui/static/card.js` (hint text), new `tools/migrate_token_brackets.py`, 13 test files + new tests, docs (ARCHITECTURE §6.3a, README, OPERATIONS_QA)
- Current behavior: MediaVault stamps the Plex-style curly token `{tmdb-12345}` on show/movie folders. Jellyfin and Emby do NOT parse curly braces at all (jellyfin#14928), so the id tag is inert in the two servers the end goal actually targets; Plex parses it but Plex is the warm fallback. The web console's suggestion placeholder offers `{tvdb-000000}` for series/anime even though MediaVault is TMDB-for-everything and has no TVDB client.
- Proposed change: Stamp the Jellyfin/Emby-native square form `[tmdbid-<id>]` (D1) via one shared formatter used by both enrich stamp sites; recognize provider tokens in BOTH shapes (square `[tmdbid-|tvdbid-|imdbid-…]` + legacy curly `{tmdb-…}`, case-insensitive, IMP-C23 generalized) so legacy folders are never double-stamped (D3); unify the suggestion placeholder on `[tmdbid-…]` (D2); artwork ancestor walk accepts any provider shape; write `movie.nfo`/`tvshow.nfo` at every stamp (D6 — reuses `_write_nfo`, never overwrites an existing NFO, `--no-nfo` opt-out) so Plex pins the same id via its official NFO agent (PMS 1.43+) with a clean folder name; one-shot `tools/migrate_token_brackets.py` (dry-run default, sequential `rename_folder` loop) converts every existing folder and backfills NFOs.
- Rationale: `[tmdbid-…]` is the only token format parsed natively by both Jellyfin and Emby; Plex's official NFO agent (2025) closes its gap without the curly tag; the user's own pre-Jellyfin folders already used square-bracket tags (`Dark (2017) [tvdbid-334824]`). Decision record + evidence: `docs/feature-token-brackets/PLAN.md` (D1–D6 all user-ruled 2026-09-07).
- Goal: Every show/movie folder carries a token all three servers understand — name for Jellyfin/Emby, NFO for Plex — with zero double-stamps and a verified live-library migration.
- Effort estimate: medium (predicate + stamp sites + ~145 test literals + migration tool + live rename)
- Risk: low-medium — touches the enrich stamp path and ~145 test literals, but no rollback/PONR/`ENTRY_TYPE_KEYS` change; migration is journal-backed per-folder via `rename_folder`, strictly sequential (IMP-C24 discipline), 🚦 user-gated on the dry-run report. Plex loses its curly-tag hint on migrated folders (superseded by the NFO pin; per-folder fallback possible).
- If skipped: the id tag stays invisible to Jellyfin/Emby (the end-goal primary), every new stamp keeps Plex-only value, and the future Jellyfin stand-up (IMP-S1) indexes un-pinnable names.
- Status: done (2026-09-07, PR #51) — code + tests + docs merged into the branch; the LIVE library migration was also executed with `tools/migrate_token_brackets.py` (199 folders renamed across Phase A+B + 200 NFOs backfilled; 1 Friends show-parent remains under a Windows file lock — the idempotent tool retries it anytime). Full record: `docs/feature-token-brackets/` (PLAN with ruled D1/D2/D6, DECISIONS, PROGRESS run history, dry-run + apply logs).
