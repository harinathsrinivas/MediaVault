...
## IMP-U6: canonical `[tmdbid-<id>]` provider-token folder format + `migrate_provider_tokens`
...
- Proposed change (SHIPPED on `feature/imp_u6_provider_tokens`):
  ...
  - All emission (rename_folder token stamping, enrich_metadata) now writes the canonical `[tmdbid-<id>]` bracket form, matching the ~1,400 folders an external migration already renamed and matching what Emby/Jellyfin document as their recognized format.
  ...
  - **Known tradeoff, accepted by user decision D1:** `[tmdbid-…]` is read by Emby and Jellyfin; Plex ignores bracketed folder content entirely and falls back to fuzzy title/year matching. Dual-token emission (writing both forms) was considered and rejected in favor of single-token canonical form; it remains a one-line change if Plex support is ever needed.
...
- Goal: A single shared implementation is the only place that recognizes or emits provider-token folder formats, `[tmdbid-<id>]` is the format all NEW tokens are written in, and the existing library can be safely converted to match with a dry-run-first migration command.
...
- Status: **done** on `feature/imp_u6_provider_tokens` — full suite 938/938 passed, smoke 81/81 passed; PR to `main` pending (not yet merged).
