...
def test_apply_movie_writes_tmdb_id_stamps_token_and_downloads(sandbox, patch_tmdb, capsys):
    ...
    new_folder = folder.parent / f"F1 {mvcommon.CANONICAL_TMDB_TOKEN_FMT.format(id=1003159)}"
...
def test_show_centric_two_seasons_one_resolve_one_stamp(sandbox, patch_tmdb, capsys):
    ...
    stamped = show["show_root"].parent / f"The Office {mvcommon.CANONICAL_TMDB_TOKEN_FMT.format(id=2316)}"
    ...
    assert not (stamped / f"Season 01 {mvcommon.CANONICAL_TMDB_TOKEN_FMT.format(id=2316)}").exists()
...
def test_ambiguous_match_is_listed_not_written(sandbox, patch_tmdb, capsys):
    ...
    assert not (folder.parent / f"The Thing {mvcommon.CANONICAL_TMDB_TOKEN_FMT.format(id=1091)}").exists()
    assert not (folder / "poster.jpg").exists()
...
def test_tmdb_error_on_one_unit_is_skipped_without_corruption(sandbox, patch_tmdb, capsys):
    ...
    assert lib["mov-en-2025-f1"]["metadata"]["tmdb_id"] == 1003159
    assert (f_ok.parent / f"F1 {mvcommon.CANONICAL_TMDB_TOKEN_FMT.format(id=1003159)}").is_dir()
...
def test_idempotent_rerun_does_not_double_stamp(sandbox, patch_tmdb, capsys):
    ...
    assert new_folder.is_dir()
    assert not (folder.parent / f"{new_folder.name} {mvcommon.CANONICAL_TMDB_TOKEN_FMT.format(id=1003159)}").exists()
...
def test_ambiguous_no_confident_lists_and_writes_nothing(sandbox, patch_tmdb):
    ...
    assert not (folder.parent / f"Some Obscure Film {mvcommon.CANONICAL_TMDB_TOKEN_FMT.format(id=11)}").exists()
...
def test_dry_run_with_preset_id_prints_by_id_intent_writes_nothing(sandbox, patch_tmdb, capsys):
    ...
    assert mvcommon.load_library() == before
    assert folder.exists()
    assert not (folder.parent / f"F1 {mvcommon.CANONICAL_TMDB_TOKEN_FMT.format(id=1003159)}").exists()
    assert not (folder / "poster.jpg").exists()
...
def test_nfo_show_written_on_apply_with_flag(sandbox, patch_tmdb, capsys):
    ...
    stamped_show = show["show_root"].parent / f"The Office {mvcommon.CANONICAL_TMDB_TOKEN_FMT.format(id=2316)}"
...
def test_nfo_not_written_in_dry_run(sandbox, patch_tmdb, capsys):
    ...
    assert not (folder / "movie.nfo").exists()
    assert not (folder.parent / f"F1 {mvcommon.CANONICAL_TMDB_TOKEN_FMT.format(id=1003159)}" / "movie.nfo").exists()
    out = capsys.readouterr().out
    assert "DRY-RUN" in out
...
def test_api_miss_then_exa_fallback_resolves_confident(sandbox, patch_tmdb, monkeypatch, capsys):
    ...
    assert (folder.parent / f"Vaaranam Aayiram {mvcommon.CANONICAL_TMDB_TOKEN_FMT.format(id=38637)}").is_dir()
    assert not folder.exists()
