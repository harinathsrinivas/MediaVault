...
def test_nested_layout_happy_path_stamps_parent_folder_with_note(
    ...
    stamped_show = show_dir.parent / f"Dark {mvcommon.CANONICAL_TMDB_TOKEN_FMT.format(id=TMDB_ID)}"
...
def test_flat_layout_happy_path_stamps_same_folder_no_note(
    ...
    stamped = season_dir.parent / f"Peaky.Blinders.S06.2022 {mvcommon.CANONICAL_TMDB_TOKEN_FMT.format(id=TMDB_ID)}"
...
def test_sibling_season_preset_not_reached_by_base_id_scoping(
    ...
    assert not (sibling_dir.parent / f"Peaky.Blinders.S05.2019 {mvcommon.CANONICAL_TMDB_TOKEN_FMT.format(id=TMDB_ID_A)}").exists()
...
def test_show_nfo_carries_creator_names_from_created_by(
    ...
    assert result is True
    stamped = season_dir.parent / f"Stranger.Things.S01.2016 {mvcommon.CANONICAL_TMDB_TOKEN_FMT.format(id=TMDB_ID)}"
...
def test_inventory_flat_layout_full_artifact_checklist(
    ...
    assert rename_calls[0][1] == f"Peaky.Blinders.S06.2022 {mvcommon.CANONICAL_TMDB_TOKEN_FMT.format(id=TMDB_ID)}"
    assert stamped.is_dir() and not season_dir.exists()
    assert "this is the SHOW folder" not in out, \
...
def test_episode_range_that_fully_archives_still_stamps_the_token(
    ...
    stamped = season_dir.parent / f"TheExpanse.S05.2020 {mvcommon.CANONICAL_TMDB_TOKEN_FMT.format(id=TMDB_ID)}"
    assert stamped.is_dir() and not season_dir.exists()
...
def test_mid_season_failure_skips_enrich_and_leaves_resume_message_untouched(
    ...
    assert lib[f"{base_id}e01"]["status"] == "archived"
    assert lib[f"{base_id}e02"]["status"] == "local_ready"
    assert not (season_dir.parent / f"Halted.S01.2020 {mvcommon.CANONICAL_TMDB_TOKEN_FMT.format(id=60574)}").exists()
...
def test_nfo_is_regenerated_not_kept(
    ...
    nfo = (show_dir.parent / f"Dark {mvcommon.CANONICAL_TMDB_TOKEN_FMT.format(id=TMDB_ID)}" / "tvshow.nfo").read_text(encoding="utf-8")
...
def test_default_rename_choice_is_non_interactive_and_does_not_rename(
    ...
    assert not (season_dir.parent / f"Peaky.Blinders.S06.2022 {mvcommon.CANONICAL_TMDB_TOKEN_FMT.format(id=TMDB_ID)}").exists()
...
def test_anime_glued_season_episode_ids_archive_and_enrich_via_tv_endpoints(
    ...
    stamped_show = show_dir.parent / f"Kurokos Basketball {mvcommon.CANONICAL_TMDB_TOKEN_FMT.format(id=TMDB_ID)}"
...
def test_anime_without_a_season_suffix_resolves_through_search_tv(
    ...
    stamped = season_dir.parent / f"Attack on Titan {mvcommon.CANONICAL_TMDB_TOKEN_FMT.format(id=TMDB_ID)}"
    assert stamped.is_dir() and not season_dir.exists()
